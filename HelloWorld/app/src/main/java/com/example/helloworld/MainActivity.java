...
//import android.support.v7.app.AppCompatActivity;用上一句替换
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    Button btn_0,btn_1,btn_2,btn_3,btn_4,btn_5,btn_6,
            ...
            btn_sub,btn_add,btn_equ,btn_p,btn_tan,btn_lifang;
    EditText text;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_lifang = (Button) findViewById(R.id.btn_lifang);
        btn_lifang.setOnClickListener(this);
    ...
    public void onClick(View v) {
        ...
        switch(v.getId()){
            ...
            case R.id.btn_p:
                ...
                break;
            case R.id.btn_add:           //如果最后一个字符已经是+，不能再输入符号
                if(str.charAt(str.length()-1)=='+'||str.charAt(str.length()-1)=='-'||str.charAt(str.length()-1)=='×'||str.charAt(str.length()-1)=='÷'||str.charAt(str.length()-1)=='.'){
                    Toast.makeText(MainActivity.this,"Input error!",Toast.LENGTH_SHORT).show();
                    text.setText(str);
                }
            ...
            case R.id.btn_equ:
                ...
                break;
            case R.id.btn_squ2:
                if(str.charAt(0)=='-'){            //如果第一个字符是-，提示负数不能开根号，结果归0
                    Toast.makeText(MainActivity.this,"Negative numbers cannot be squared!",Toast.LENGTH_SHORT).show();
                    text.setText("0");
                }
                else if(str.contains("+")||str.contains("-")||str.contains("×")||str.contains("÷")){  //如果输入的最后一个是符号，提示符号不能开根号，结果归0
            ...
            case R.id.btn_cos:
                ...
                break;
            case R.id.btn_PandM:
                if(str.charAt(0)>='0'&&str.charAt(0)<='9'){//正数加负号，0不变
                    if(str.equals("0")){
                        text.setText("0");
                    }
                    else{
            ...
            default:
                break;
            case R.id.btn_tan:
                double ttan = Double.parseDouble(str);
                ttan=Math.toRadians(ttan);
                ttan=Math.tan(ttan);
                String ttan1=String.format("%.9f",ttan);//规避极小误差
                ttan1 = ttan1.replaceAll("0+?$", "");//去掉多余的0
                ttan1 = ttan1.replaceAll("[.]$", "");//如最后一位是.则去掉
                String t1=String.format("90");
                if(t1.equals(str)) {
                ...
                break;
            case R.id.btn_lifang:
                double lifang = Double.parseDouble(str);
                lifang=Math.pow(lifang,3);
                String lifang1=String.format("%.9f",lifang);//规避极小误差,保留9位小数
                lifang1 = lifang1.replaceAll("0+?$", "");//去掉多余的0
                lifang1 = lifang1.replaceAll("[.]$", "");//如最后一位是.则去掉
                text.setText(lifang1);
                break;
        }
    }/*
    //将中缀表达式转换成后缀表达式
    public static StringBuffer toPostfix(String infix){
        ...
        StringBuffer fushu=new StringBuffer(infix.length()*2);   //后缀表达式负数标识
        int i=0;
        while(i<infix.length()){
            char ch=infix.charAt(i);
            if(!stack.isEmpty()&&stack.peek().equals("-")&&i==0){ //如果栈不为空且栈顶元素是"-",则出栈
                fushu.append(stack.pop());   //且添加到后缀表达式串   -------------------------------------
